...
use super::Error;
use crate::{
	mock::{AssetId as RegistryAssetId, *},
	types::{AssetDetails, AssetMetadata, AssetType, Metadata},
	Event, XcmRateLimitsInRegistry,
};
use codec::Encode;
...
fn register_asset_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(AssetRegistryPallet::asset_ids(&bn).unwrap(), 1u32 + SequentialIdStart::get());
		assert_eq!(
...
fn create_asset() {
	new_test_ext().execute_with(|| {
		...
		let dot: BoundedVec<u8, <Test as crate::Config>::StringLimit> =
			b"DOT".to_vec().try_into().unwrap();
		let aaa: BoundedVec<u8, <Test as crate::Config>::StringLimit> =
			b"AAA".to_vec().try_into().unwrap();

		assert_eq!(AssetRegistryPallet::asset_ids(dot).unwrap(), 2u32 + SequentialIdStart::get());
...
fn location_mapping_works() {
	new_test_ext().execute_with(|| {
		...
		let asset_id: RegistryAssetId =
			AssetRegistryPallet::get_or_create_asset(b"BSX".to_vec(), AssetType::Token, ed, None)
				.unwrap();
		...
		assert_eq!(AssetRegistryPallet::location_to_asset(asset_location.clone()), Some(asset_id));
...
fn genesis_config_works() {
	...
	ExtBuilder::default()
		.with_assets(vec![(one.clone(), 1_000u128, None), (life.clone(), 1_000u128, Some(42))])
		.build()
		.execute_with(|| {
			let native: BoundedVec<u8, <Test as crate::Config>::StringLimit> =
				b"NATIVE".to_vec().try_into().unwrap();
			...
			let bsx: BoundedVec<u8, <Test as crate::Config>::StringLimit> =
			...
			let life: BoundedVec<u8, <Test as crate::Config>::StringLimit> =
				life.try_into().unwrap();
...
fn set_metadata_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let b_symbol: BoundedVec<u8, <Test as crate::Config>::StringLimit> =
			...
			assert_eq!(
				...
				AssetMetadata { decimals: 12u8, symbol: b_symbol.clone() }
			);
...
fn update_asset() {
	new_test_ext().execute_with(|| {
		...
		let btc_asset_id: RegistryAssetId =
			AssetRegistryPallet::get_or_create_asset(b"BTC".to_vec(), AssetType::Token, ed, None)
				.unwrap();
		let usd_asset_id: RegistryAssetId =
			AssetRegistryPallet::get_or_create_asset(b"USD".to_vec(), AssetType::Token, ed, None)
				.unwrap();
		...
		// corner case: change the name and also the type for an existing asset (token -> pool
		// share)
		assert_ok!(AssetRegistryPallet::update(
			RuntimeOrigin::root(),
			btc_asset_id,
		...
		// corner case: change the name and also the type for an existing asset (pool share ->
		// token)
		assert_ok!(AssetRegistryPallet::update(
			RuntimeOrigin::root(),
			btc_asset_id,
...
fn get_ed_by_key_works() {
	ExtBuilder::default()
		.with_native_asset_name(b"NATIVE".to_vec())
		.with_assets(vec![(b"ONE".to_vec(), 1_000u128, None), (b"TWO".to_vec(), 2_000u128, None)])
		.build()
		.execute_with(|| {
...
fn register_asset_should_work_when_metadata_is_provided() {
	new_test_ext().execute_with(|| {
		...
		expect_events(vec![
			...
			.into(),
			Event::MetadataSet { asset_id, symbol: b_symbol.clone(), decimals }.into(),
		]);
		...
		assert_eq!(
			...
			AssetMetadata { decimals: 18u8, symbol: b_symbol }
		);
	});
}
...
fn register_asset_should_work_when_all_optional_are_provided() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(AssetRegistryPallet::register(
			...
			1_000_000,
			Some(asset_id),
			Some(Metadata { symbol: b"SYM".to_vec(), decimals: 18 }),
			Some(asset_location.clone()),
			Some(1000 * UNIT)
		),);
