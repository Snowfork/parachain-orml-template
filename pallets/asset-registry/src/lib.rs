...
use frame_support::{pallet_prelude::*, sp_runtime::traits::CheckedAdd};
...
use primitives::{AssetKind, CreateRegistry, InspectRegistry, Registry};
...
pub mod pallet {
	...
	#[pallet::error]
	pub enum Error<T> {
		/// Asset ID is not available. This only happens when it reaches the MAX value of given id
		/// type.
		NoIdAvailable,
	...
	impl<T: Config> Pallet<T> {
		...
		///
		/// Adds mapping between `name` and assigned `asset_id` so asset id can be retrieved by name
		/// too (Note: this approach is used in AMM implementation (xyk))
		///
		...
		pub fn register(
			...
			if let Some(meta) = metadata {
				...
				AssetMetadataMap::<T>::insert(
					asset_id,
					AssetMetadata { symbol: symbol.clone(), decimals: meta.decimals },
				);
			...
			if let Some(loc) = location {
				...
				Self::deposit_event(Event::LocationSet { asset_id, location: loc });
			}
		...
		///
		/// Updates also mapping between name and asset id if provided name is different than
...
impl<T: Config> Pallet<T> {
	...
	/// Convert Vec<u8> to BoundedVec so it respects the max set limit, otherwise return TooLong
	/// error
	...
	/// Register new asset.
	///
	/// Does not perform any  check whether an asset for given name already exists. This has to be
	...
	pub fn register_asset(
		...
		let asset_id = if let Some(selected_id) = selected_asset_id {
			ensure!(selected_id < T::SequentialIdStartAt::get(), Error::<T>::NotInReservedRange);

			ensure!(!Assets::<T>::contains_key(selected_id), Error::<T>::AssetAlreadyRegistered);
			...
			NextAssetId::<T>::mutate(|value| -> Result<T::AssetId, DispatchError> {
				...
				// Note: this way we prevent accidental clashes with native asset id, so no need to
				// set next asset id to be > next asset id
		...
		let details =
			AssetDetails { name: name.clone(), asset_type, existential_deposit, xcm_rate_limit };
		...
		Self::deposit_event(Event::Registered { asset_id, asset_name: name, asset_type });
...
impl<T: Config> Registry<T::AssetId, Vec<u8>, T::Balance, DispatchError> for Pallet<T> {
	...
	fn retrieve_asset_type(asset_id: T::AssetId) -> Result<AssetKind, DispatchError> {
		let asset_details = Assets::<T>::get(asset_id)
			.ok_or_else(|| Into::<DispatchError>::into(Error::<T>::AssetNotFound))?;
