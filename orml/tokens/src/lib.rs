...
pub mod module {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn transfer_all(
			...
			let to = T::Lookup::lookup(dest)?;
			let preservation =
				if keep_alive { Preservation::Protect } else { Preservation::Expendable };
				...
				|from_account, _existed| -> DispatchResult {
					...
					let would_be_dead = if from_account.total() < ed {
						if from_account.total().is_zero() {
							...
							// Note: if account is not in `T::DustRemovalWhitelist`, account will
							// eventually be reaped due to the dust removal.
...
impl<T: Config> fungibles::MutateHold<T::AccountId> for Pallet<T> {
	...
	fn transfer_on_hold(
		...
		let status =
			if restriction == Restriction::OnHold { Status::Reserved } else { Status::Free };
		ensure!(
			amount <=
